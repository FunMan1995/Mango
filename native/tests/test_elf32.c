...
static int test_parses_a_valid_image(void) {
  ...
  if (image.segments[0].vaddr != 0 || image.segments[0].filesz != 0x114 ||
      !image.segments[0].executable) {
    fprintf(stderr,
            "FAIL(parses_a_valid_image): segment fields wrong (vaddr=%u filesz=%u exec=%d)\n",
...
static uint32_t test_resolve_malloc(void* ctx, const char* name, uint32_t st_value,
                                    uint32_t st_shndx) {
  (void)ctx;
  (void)st_value;
  (void)st_shndx;
  if (name != NULL && strcmp(name, "malloc") == 0) {
    return 0x2000u;
  }
  return 0;
}

static int test_applies_relative_and_jump_slot(void) {
  /* Reloc table at 0: RELATIVE to 16, JUMP_SLOT to 20. Payload words follow. */
  uint8_t mem[32];
  memset(mem, 0, sizeof(mem));
  mem[0] = 16; /* r_offset = 16 */
  mem[4] = (uint8_t)MANGO_R_ARM_RELATIVE;
  mem[8] = 20; /* r_offset = 20 */
  mem[12] = (uint8_t)MANGO_R_ARM_JUMP_SLOT;
  mem[13] = 1; /* sym index 1 in r_info high bytes: 1 << 8 */
  mem[16] = 0x00;
  mem[17] = 0x01; /* addend 0x100 */
  MangoElf32Image image;
  memset(&image, 0, sizeof(image));
  image.data = kSynthElf;
  image.size = (uint32_t)sizeof(kSynthElf);
  if (mango_elf32_parse(kSynthElf, sizeof(kSynthElf), EM_ARM_FOR_TESTS, &image) != 0) {
    fprintf(stderr, "FAIL(applies_relocs): parse of synth ELF failed\n");
    return 1;
  }
  image.rel_vaddr = 0;
  image.rel_size = 16;
  /* Reuse synth .dynsym index 1 = mango_add (defined). For JUMP_SLOT with
  ...
  uint32_t rel = (uint32_t)mem[16] | ((uint32_t)mem[17] << 8) | ((uint32_t)mem[18] << 16) |
                 ((uint32_t)mem[19] << 24);
...
int main(void) {
  ...
  failures += test_applies_relative_and_jump_slot();
