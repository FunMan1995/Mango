...
static int test_t32_add_strd_reject(void) {
  /* Negatives: S=1 ADD uncover; undefined STRD P=0 W=0; ADDW untouched.
   * W=1 STRD e9e1 now open via Q-OTTD-0t widen. */
  MangoInsn di;
  ...
  printf("ok: T32 ADD reject S=1; STRD reject P=0 W=0; ADDW #0xD08 untouched (Q-OTTD-0e/0t)\n");
  return 0;
}
...
static int test_t32_ldrb_w_imm8_contrasts(void) {
  ...
  /* T32 CLZ fab0 f080 still uncover (footnote) */
  if (mango_decode_t32(0xFAB0u, 0xF080u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_ldrb_w_contrasts): T32 CLZ fab0f080 should remain uncover\n");
    return 1;
  }
  printf("ok: T32 LDRB.W imm8 contrasts + reject LDRBT/PC; imm12/0m tip; LDRH tip; CLZ uncover "
         "(Q-OTTD-0aa)\n");
  return 0;
}
...
static int test_t32_ldrh_w_imm8_contrasts(void) {
  ...
  /* Optional hold: STRH.W imm8 f824 1f02 still uncover */
  if (mango_decode_t32(0xF824u, 0x1F02u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_ldrh_w_contrasts): STRH imm8 f8241f02 should remain uncover\n");
    return 1;
  }
  /* T32 CLZ fab0 f080 still uncover (footnote) */
  if (mango_decode_t32(0xFAB0u, 0xF080u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_ldrh_w_contrasts): T32 CLZ fab0f080 should remain uncover\n");
    return 1;
  }
  printf("ok: T32 LDRH.W imm8 contrasts + reject LDRHT/PC/WB-Rt; F8B0 tip; CLZ/STRH uncover "
         "(Q-OTTD-0ab)\n");
  return 0;
}
...
static int test_t32_strb_w_reg_imm2_contrasts(void) {
  ...
  /* hold next: STRH.W imm8 f823 2c08 still uncover */
  if (mango_decode_t32(0xF823u, 0x2C08u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_strb_w_reg_contrast): hold STRH imm8 f8232c08 decoded as op=%d\n",
            di.op);
    return 1;
  }
  /* footnote: T32 CLZ fab0 f080 still uncover */
  if (mango_decode_t32(0xFAB0u, 0xF080u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_strb_w_reg_contrast): CLZ fab0f080 decoded as op=%d (footnote)\n",
            di.op);
    return 1;
  }
  ...
  printf("ok: T32 STRB.W reg imm2=1 + tip 0z/imm12; hold STRH imm8/CLZ; reject PC (Q-OTTD-0ad)\n");
  return 0;
}
...
static int test_t32_and_w_modimm_reject(void) {
  ...
  /* AND register ea03 0301 stays uncover */
  if (mango_decode_t32(0xEA03u, 0x0301u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_and_w_reject): AND-reg ea030301 decoded as op=%d\n", di.op);
    return 1;
  }
  /* footnote CLZ fab0 f080 */
  if (mango_decode_t32(0xFAB0u, 0xF080u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_and_w_reject): CLZ fab0f080 decoded as op=%d\n", di.op);
    return 1;
  }
  /* optional hold STRH.W imm8 f824 1f02 */
  if (mango_decode_t32(0xF824u, 0x1F02u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_and_w_reject): STRH f8241f02 decoded as op=%d\n", di.op);
    return 1;
  }
  /* 0ab LDRH still tip */
  ...
  printf("ok: T32 AND.W reject PC; BIC/LDRH tip; CLZ/STRH uncover (Q-OTTD-0ac)\n");
  return 0;
}
...
static int test_t32_mov_w_reg_reject(void) {
  /* MOVS ea5f, ORR ea41, MVN-reg ea6f stay uncover; Rd/Rm=PC reject.
   * ADD.W eb0e and BIC f026 still tip. */
  MangoInsn di;
  if (mango_decode_t32(0xEA5Fu, 0x7AD0u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_mov_w_reg_reject): MOVS ea5f7ad0 decoded as op=%d\n", di.op);
    return 1;
  }
  if (mango_decode_t32(0xEA41u, 0x70D2u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_mov_w_reg_reject): ORR ea4170d2 decoded as op=%d\n", di.op);
    return 1;
  }
  if (mango_decode_t32(0xEA6Fu, 0x70D1u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_mov_w_reg_reject): MVN-reg ea6f70d1 decoded as op=%d\n", di.op);
    return 1;
  }
  ...
  printf("ok: T32 MOV.W reg reject MOVS/ORR/MVN/PC; ADD+BIC ok (Q-OTTD-0r)\n");
  return 0;
}
