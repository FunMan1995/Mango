...
static int test_t32_and_w_reg(void) {
   ...
   * Flags hold. ANDS (ea1b) is Q-OTTD-0cu. lsl #4 is ea0b 1303. */
  ...
  if (mango_decode_t32(0xEA1Bu, 0x0303u, &di) != 0 || di.op != MANGO_OP_AND || di.sets_flags != 1) {
    fprintf(stderr, "FAIL(t32_and_reg): ANDS ea1b0303 is Q-OTTD-0cu\n");
    return 1;
  }
  if (mango_decode_t32(0xEA0Fu, 0x0303u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_and_reg): Rn=PC decoded\n");
    return 1;
  }
...
static int test_t32_ands_w_reg(void) {
  /* Q-OTTD-0cu: ands.w r6, r8, r6 = ea18 0606 (llvm-mc [18,ea,06,06]).
   * r6 = r8 & r6. LSL #0 leaves C. V holds. Rd overlaps Rm.
   * lsl #1 is ea18 0646: C is bit 31 of Rm. */
  static const uint16_t kProg[] = {0xEA18u, 0x0606u, 0x4770u};
  static const uint16_t kLsl[] = {0xEA18u, 0x0646u, 0x4770u};
  uint8_t mem_buf[32];
  uint8_t lsl_buf[32];
  load_halfwords(mem_buf, sizeof(mem_buf), kProg, 3);
  load_halfwords(lsl_buf, sizeof(lsl_buf), kLsl, 3);
  struct {
    uint32_t r8, r6, extra, want, nzcv;
  } cases[] = {
      {0xff00u, 0x0ff0u, MANGO_CPSR_C | MANGO_CPSR_V, 0x0f00u, MANGO_CPSR_C | MANGO_CPSR_V},
      {0u, 0xffffffffu, MANGO_CPSR_C | MANGO_CPSR_V, 0u, MANGO_CPSR_Z | MANGO_CPSR_C | MANGO_CPSR_V},
      {0x80000000u, 0xffffffffu, MANGO_CPSR_V, 0x80000000u, MANGO_CPSR_N | MANGO_CPSR_V},
      {0xffffffffu, 0xffu, MANGO_CPSR_Z | MANGO_CPSR_C, 0xffu, MANGO_CPSR_C},
  };
  MangoInsn di;
  if (mango_decode_t32(0xEA18u, 0x0606u, &di) != 0 || di.op != MANGO_OP_AND || di.rd != 6 ||
      di.rn != 8 || di.rm != 6 || di.is_imm != 0 || di.sets_flags != 1 || di.shift_amount != 0 ||
      di.shift_type != 0) {
    fprintf(stderr, "FAIL(t32_ands_w): decode op=%d rd=%u rn=%u rm=%u s=%d amt=%u\n", di.op, di.rd,
            di.rn, di.rm, di.sets_flags, di.shift_amount);
    return 1;
  }
  if (mango_decode_t32(0xEA18u, 0x0646u, &di) != 0 || di.shift_amount != 1 || di.shift_type != 0 ||
      di.sets_flags != 1) {
    fprintf(stderr, "FAIL(t32_ands_w): lsl #1 amt=%u type=%u s=%d\n", di.shift_amount, di.shift_type,
            di.sets_flags);
    return 1;
  }
  if (mango_decode_t32(0xEA18u, 0x1616u, &di) != 0 || di.shift_amount != 4 || di.shift_type != 1) {
    fprintf(stderr, "FAIL(t32_ands_w): lsr #4 amt=%u type=%u\n", di.shift_amount, di.shift_type);
    return 1;
  }
  if (mango_decode_t32(0xEA08u, 0x0606u, &di) != 0 || di.sets_flags != 0 || di.op != MANGO_OP_AND) {
    fprintf(stderr, "FAIL(t32_ands_w): AND.W S=0 should stay flags-off\n");
    return 1;
  }
  if (mango_decode_t32(0xEA18u, 0x0F06u, &di) != 0 || di.op != MANGO_OP_TST) {
    fprintf(stderr, "FAIL(t32_ands_w): Rd=15 should stay TST op=%d\n", di.op);
    return 1;
  }
  if (mango_decode_t32(0xEA1Fu, 0x0606u, &di) == 0 || mango_decode_t32(0xEA18u, 0x060Fu, &di) == 0) {
    fprintf(stderr, "FAIL(t32_ands_w): PC decoded\n");
    return 1;
  }
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    MangoCpu cpu;
    ...
    if (rc != 0 || cpu.r[6] != cases[c].want || cpu.r[8] != cases[c].r8 || nzcv != cases[c].nzcv) {
      fprintf(stderr, "FAIL(t32_ands_w#%u): rc=%d r6=%x want %x nzcv=%x want %x\n", c, rc, cpu.r[6],
              cases[c].want, nzcv, cases[c].nzcv);
      return 1;
    }
  }
  {
    ...
    cpu.r[6] = 0x80000000u;
    cpu.r[MANGO_REG_LR] = 0xABCDu;
    MangoMemory mem = {lsl_buf, sizeof(lsl_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    uint32_t nzcv = cpu.cpsr & (MANGO_CPSR_N | MANGO_CPSR_Z | MANGO_CPSR_C | MANGO_CPSR_V);
    if (rc != 0 || cpu.r[6] != 0 || cpu.r[8] != 0xffffffffu ||
        nzcv != (MANGO_CPSR_Z | MANGO_CPSR_C | MANGO_CPSR_V)) {
      fprintf(stderr, "FAIL(t32_ands_w lsl1): rc=%d r6=%x nzcv=%x\n", rc, cpu.r[6], nzcv);
      return 1;
    }
  }
  printf("ok: T32 ANDS.W r6, r8, r6 (Q-OTTD-0cu)\n");
  return 0;
}
...
static int test_t32_tst_w_reg(void) {
  ...
  if (mango_decode_t32(0xEA18u, 0x0103u, &di) != 0 || di.op != MANGO_OP_AND || di.rd != 1 ||
      di.sets_flags != 1) {
    fprintf(stderr, "FAIL(t32_tst_w): ANDS ea180103 is Q-OTTD-0cu\n");
    return 1;
  }
  if (mango_decode_t32(0xEA1Fu, 0x0F03u, &di) == 0 || mango_decode_t32(0xEA18u, 0x0F0Fu, &di) == 0) {
    fprintf(stderr, "FAIL(t32_tst_w): PC decoded\n");
    return 1;
  }
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    MangoCpu cpu;
...
static int test_t32_lsl_w_reg(void) {
  ...
  if (mango_decode_t32(0xFA20u, 0xF204u, &di) != 0 || di.shift_type != 1 || di.sets_flags != 0) {
    fprintf(stderr, "FAIL(t32_lsl_w): LSR.W S=0 should decode\n");
    return 1;
  }
...
static int test_t32_asr_w_reg(void) {
  ...
  if (mango_decode_t32(0xFA51u, 0xF20Bu, &di) == 0) {
    fprintf(stderr, "FAIL(t32_asr): ASRS decoded\n");
    return 1;
  }
  if (mango_decode_t32(0xFA21u, 0xF20Bu, &di) != 0 || di.shift_type != 1 || di.sets_flags != 0) {
    fprintf(stderr, "FAIL(t32_asr): LSR.W S=0 should decode\n");
    return 1;
  }
...
static int test_t32_ldrsb_w_imm8(void) {
   ...
   * can be the base. LDRSBT, Rt/Rn=PC, and WB into Rt stay closed.
   * The register form is Q-OTTD-0cz. */
  ...
  if (mango_decode_t32(0xF91Eu, 0x4A01u, &di) == 0 || mango_decode_t32(0xF91Eu, 0xFF01u, &di) == 0 ||
    ...
    fprintf(stderr, "FAIL(t32_ldrsb_imm8): LDRSBT, PC, or WB-into-Rt decoded\n");
    return 1;
  }
...
static int test_t32_ldrsb_w_reg(void) {
  /* Q-OTTD-0cz: ldrsb.w r1, [r1, r2, lsl #1] = f911 1012
   * (llvm-mc [11,f9,12,10]) in Aircraft::Tick. Rt equals Rn, so r1
   * becomes the sign-extended byte and r2 holds. lsl #0 is f911 1002,
   * lsl #2 is f911 1022, lsl #3 is f911 1032. Non-aliasing
   * f910 2011 keeps r0. f91e 4001 is ldrsb.w r4, [lr, r1]. */
  struct {
    uint16_t hw1, hw2;
    uint32_t rn, rm, rd;
    uint32_t base, index, shift, off;
    uint8_t byte;
    uint32_t want;
    int alias;
  } cases[] = {
      {0xF911u, 0x1012u, 1u, 2u, 1u, 0x40u, 3u, 1u, 6u, 0x80u, 0xffffff80u, 1},
      {0xF911u, 0x1002u, 1u, 2u, 1u, 0x40u, 5u, 0u, 5u, 0x7fu, 0x7fu, 1},
      {0xF911u, 0x1022u, 1u, 2u, 1u, 0x40u, 1u, 2u, 4u, 0x00u, 0u, 1},
      {0xF911u, 0x1032u, 1u, 2u, 1u, 0x40u, 2u, 3u, 16u, 0xfeu, 0xfffffffeu, 1},
      {0xF910u, 0x2011u, 0u, 1u, 2u, 0x40u, 3u, 1u, 6u, 0x80u, 0xffffff80u, 0},
  };
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    uint8_t mem_buf[128];
    ...
    if (mango_decode_t32(cases[c].hw1, cases[c].hw2, &di) != 0 || di.op != MANGO_OP_LDRSB ||
        di.rd != cases[c].rd || di.rn != cases[c].rn || di.rm != cases[c].rm || di.is_imm != 0 ||
        di.shift_type != 0 || di.shift_amount != cases[c].shift || di.p != 1 || di.u != 1 ||
        di.w != 0) {
      fprintf(stderr, "FAIL(t32_ldrsb_w_reg#%u): decode op=%d rd=%u rn=%u rm=%u sh=%u\n", c, di.op,
              di.rd, di.rn, di.rm, di.shift_amount);
      return 1;
    }
    mem_buf[cases[c].base + cases[c].off] = cases[c].byte;
    MangoCpu cpu;
    memset(&cpu, 0, sizeof(cpu));
    cpu.cpsr = MANGO_CPSR_T | MANGO_CPSR_N | MANGO_CPSR_C;
    uint32_t cpsr_before = cpu.cpsr;
    cpu.r[cases[c].rn] = cases[c].base;
    cpu.r[cases[c].rm] = cases[c].index;
    if (!cases[c].alias) {
      cpu.r[cases[c].rd] = 0x11111111u;
    }
    cpu.r[MANGO_REG_LR] = 0xABCDu;
    MangoMemory mem = {mem_buf, sizeof(mem_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    uint32_t base_after = cases[c].alias ? cases[c].want : cases[c].base;
    if (rc != 0 || cpu.r[cases[c].rd] != cases[c].want || cpu.r[cases[c].rn] != base_after ||
        cpu.r[cases[c].rm] != cases[c].index || cpu.cpsr != cpsr_before) {
      fprintf(stderr, "FAIL(t32_ldrsb_w_reg#%u): rc=%d rd=%x want %x rn=%x rm=%x cpsr=%x\n", c, rc,
              cpu.r[cases[c].rd], cases[c].want, cpu.r[cases[c].rn], cpu.r[cases[c].rm], cpu.cpsr);
      return 1;
    }
  }

  MangoInsn di;
  if (mango_decode_t32(0xF91Eu, 0x4001u, &di) != 0 || di.op != MANGO_OP_LDRSB || di.rd != 4 ||
      di.rn != 14 || di.rm != 1 || di.is_imm != 0 || di.shift_amount != 0) {
    fprintf(stderr, "FAIL(t32_ldrsb_w_reg): f91e4001 op=%d rd=%u rn=%u\n", di.op, di.rd, di.rn);
    return 1;
  }
  if (mango_decode_t32(0xF91Eu, 0x4F01u, &di) != 0 || di.op != MANGO_OP_LDRSB || di.is_imm != 1 ||
      di.w != 1) {
    fprintf(stderr, "FAIL(t32_ldrsb_w_reg): imm8 f91e4f01 should stay writeback\n");
    return 1;
  }
  if (mango_decode_t32(0xF911u, 0xF012u, &di) == 0 || mango_decode_t32(0xF91Fu, 0x1012u, &di) == 0 ||
      mango_decode_t32(0xF911u, 0x101Fu, &di) == 0 || mango_decode_t32(0xF911u, 0x1042u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_ldrsb_w_reg): PC or stray shift decoded\n");
    return 1;
  }
  printf("ok: T32 LDRSB.W r1, [r1, r2, lsl #1] (Q-OTTD-0cz)\n");
  return 0;
}
...
static int test_t32_rsb_w_imm1(void) {
   ...
   * RSBS f1d0 0401 is Q-OTTD-0da. Rd/Rn=PC stay closed. */
  ...
  if (mango_decode_t32(0xF1D0u, 0x0401u, &di) != 0 || di.op != MANGO_OP_RSB || di.sets_flags != 1 ||
      di.rd != 4 || di.rn != 0 || di.imm != 1u) {
    fprintf(stderr, "FAIL(t32_rsb_w): RSBS f1d00401 is Q-OTTD-0da op=%d s=%d\n", di.op, di.sets_flags);
    return 1;
  }
  if (mango_decode_t32(0xF1C0u, 0x0F01u, &di) == 0 || mango_decode_t32(0xF1CFu, 0x0401u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_rsb_w): Rd=PC or Rn=PC decoded\n");
    return 1;
  }
...
static int test_t32_rsbs_w_imm0(void) {
  /* Q-OTTD-0da: rsbs.w r10, r0, #0 = f1d0 0a00 (llvm-mc [d0,f1,00,0a]).
   * r10 = 0 - r0. NZCV from the subtract. #1 is f1d0 0a01. */
  struct {
    uint16_t hw2;
    uint32_t imm, in, want, nzcv;
  } cases[] = {
      {0x0A00u, 0u, 0u, 0u, MANGO_CPSR_Z | MANGO_CPSR_C},
      {0x0A00u, 0u, 1u, 0xffffffffu, MANGO_CPSR_N},
      {0x0A00u, 0u, 0x80000000u, 0x80000000u, MANGO_CPSR_N | MANGO_CPSR_V},
      {0x0A00u, 0u, 0xffffffffu, 1u, 0u},
      {0x0A01u, 1u, 0u, 1u, MANGO_CPSR_C},
      {0x0A01u, 1u, 1u, 0u, MANGO_CPSR_Z | MANGO_CPSR_C},
      {0x0A01u, 1u, 2u, 0xffffffffu, MANGO_CPSR_N},
  };
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    uint8_t mem_buf[32];
    uint16_t prog[3] = {0xF1D0u, cases[c].hw2, 0x4770u};
    MangoInsn di;
    load_halfwords(mem_buf, sizeof(mem_buf), prog, 3);
    if (mango_decode_t32(0xF1D0u, cases[c].hw2, &di) != 0 || di.op != MANGO_OP_RSB || di.rd != 10 ||
        di.rn != 0 || di.is_imm != 1 || di.imm != cases[c].imm || di.sets_flags != 1) {
      fprintf(stderr, "FAIL(t32_rsbs_w#%u): decode op=%d imm=%u s=%d\n", c, di.op, di.imm,
              di.sets_flags);
      return 1;
    }
    MangoCpu cpu;
    ...
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    uint32_t nzcv = cpu.cpsr & (MANGO_CPSR_N | MANGO_CPSR_Z | MANGO_CPSR_C | MANGO_CPSR_V);
    if (rc != 0 || cpu.r[10] != cases[c].want || cpu.r[0] != cases[c].in || nzcv != cases[c].nzcv ||
        (cpu.cpsr & MANGO_CPSR_T) == 0) {
      fprintf(stderr, "FAIL(t32_rsbs_w#%u): rc=%d r10=%x want %x nzcv=%x want %x\n", c, rc, cpu.r[10],
              cases[c].want, nzcv, cases[c].nzcv);
      return 1;
    }
  }
  ...
  MangoInsn di;
  if (mango_decode_t32(0xF1C0u, 0x0A00u, &di) != 0 || di.op != MANGO_OP_RSB || di.sets_flags != 0) {
    fprintf(stderr, "FAIL(t32_rsbs_w): S=0 f1c00a00 should stay RSB s=%d\n", di.sets_flags);
    return 1;
  }
  if (mango_decode_t32(0xF1D0u, 0x0F00u, &di) == 0 || mango_decode_t32(0xF1DFu, 0x0A00u, &di) == 0 ||
      mango_decode_t32(0xEBD0u, 0x0A00u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_rsbs_w): PC or register RSBS decoded\n");
    return 1;
  }
  printf("ok: T32 RSBS.W r10, r0, #0 (Q-OTTD-0da)\n");
  return 0;
}
...
static int test_t32_ldrd_reject(void) {
  ...
  if (mango_decode_t32(0xE8F1u, 0x2302u, &di) != 0 || di.op != MANGO_OP_LDRD || di.p != 0 ||
      di.w != 1 || di.imm != 8u) {
    fprintf(stderr, "FAIL(t32_ldrd_reject): post e8f1 op=%d p=%d w=%d\n", di.op, di.p, di.w);
    return 1;
  }
  if (mango_decode_t32(0xE8D1u, 0x2302u, &di) == 0 || mango_decode_t32(0xE9F2u, 0x2302u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_ldrd_reject): P=0 W=0 or Rn-in-pair should stay closed\n");
    return 1;
  }
  if (mango_decode_t32(0xE9C1u, 0x8902u, &di) != 0 || di.op != MANGO_OP_STRD || di.rd != 8 ||
  ...
  printf("ok: T32 LDRD W=1 and post-index; P=0 W=0 closed; STRD e9c1 ok (Q-OTTD-0cj)\n");
  return 0;
}
...
static int test_t32_sxtb_w(void) {
  /* Q-OTTD-0cw: sxtb.w r9, r3 = fa4f f983 (llvm-mc [4f,fa,83,f9]).
   * r9 = sign_extend(r3[7:0]). Flags and r3 hold. ror #8 is fa4f f993.
   * SXTAB and SXTH.W stay closed. */
  static const uint16_t kProg[] = {0xFA4Fu, 0xF983u, 0x4770u};
  uint8_t mem_buf[32];
  load_halfwords(mem_buf, sizeof(mem_buf), kProg, 3);
  struct {
    uint32_t in, want;
  ...
  MangoInsn di;
  if (mango_decode_t32(0xFA4Fu, 0xF983u, &di) != 0 || di.op != MANGO_OP_XTEND || di.rd != 9 ||
      di.rm != 3 || di.rn != 15 || di.imm != 0 || di.u != 0 || di.b != 0 || di.sets_flags != 0) {
    fprintf(stderr, "FAIL(t32_sxtb): decode op=%d rd=%u rm=%u u=%d b=%d imm=%u\n", di.op, di.rd,
            di.rm, di.u, di.b, di.imm);
    return 1;
  }
  if (mango_decode_t32(0xFA4Fu, 0xFF83u, &di) == 0 || mango_decode_t32(0xFA4Fu, 0xF98Fu, &di) == 0 ||
      mango_decode_t32(0xFA43u, 0xF983u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_sxtb): PC or SXTAB decoded\n");
    return 1;
  }
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    MangoCpu cpu;
    ...
    MangoMemory mem = {mem_buf, sizeof(mem_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    if (rc != 0 || cpu.r[9] != cases[c].want || cpu.r[3] != cases[c].in || cpu.cpsr != cpsr_before) {
      fprintf(stderr, "FAIL(t32_sxtb#%u): rc=%d r9=%x want %x cpsr %x->%x\n", c, rc, cpu.r[9],
              cases[c].want, cpsr_before, cpu.cpsr);
      return 1;
    }
  }
  static const uint16_t kRot[] = {0xFA4Fu, 0xF993u, 0x4770u};
  load_halfwords(mem_buf, sizeof(mem_buf), kRot, 3);
  if (mango_decode_t32(0xFA4Fu, 0xF993u, &di) != 0 || di.imm != 8 || di.u != 0 || di.b != 0) {
    fprintf(stderr, "FAIL(t32_sxtb ror): imm=%u u=%d b=%d\n", di.imm, di.u, di.b);
    return 1;
  }
  MangoCpu cpu;
  memset(&cpu, 0, sizeof(cpu));
  ...
  MangoMemory mem = {mem_buf, sizeof(mem_buf)};
  int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
  if (rc != 0 || cpu.r[9] != 0xffffff80u || cpu.r[3] != 0x00008000u || cpu.cpsr != cpsr_before) {
    fprintf(stderr, "FAIL(t32_sxtb ror): rc=%d r9=%x cpsr %x->%x\n", rc, cpu.r[9], cpsr_before,
            cpu.cpsr);
    return 1;
  }
  printf("ok: T32 SXTB.W r9, r3 (Q-OTTD-0cw)\n");
  return 0;
}

static int test_t32_uxth_w_reject(void) {
  /* SXTH.W is Q-OTTD-0cy. SXTB.W is Q-OTTD-0cw. UXTH Rd/Rm=PC reject. */
  MangoInsn di;
  if (mango_decode_t32(0xFA0Fu, 0xFA81u, &di) != 0 || di.op != MANGO_OP_XTEND || di.rd != 10 ||
      di.rm != 1 || di.u != 0 || di.b != 1 || di.rn != 15) {
    fprintf(stderr, "FAIL(t32_uxth_w_reject): SXTH.W fa0ffa81 is Q-OTTD-0cy op=%d\n", di.op);
    return 1;
  }
  if (mango_decode_t32(0xFA4Fu, 0xFA81u, &di) != 0 || di.op != MANGO_OP_XTEND || di.rd != 10 ||
      di.rm != 1 || di.u != 0 || di.b != 0) {
    fprintf(stderr, "FAIL(t32_uxth_w_reject): SXTB.W fa4ffa81 is Q-OTTD-0cw op=%d\n", di.op);
    return 1;
  }
  ...
  printf("ok: T32 UXTH.W reject SXTH/PC; SXTB.W is 0cw (Q-OTTD-0o)\n");
  return 0;
}
...
static int test_t32_sxtah(void) {
  /* Q-OTTD-0cx: sxtah r3, r0, r3 = fa00 f383 (llvm-mc [00,fa,83,f3]).
   * r3 = r0 + sign-extended low halfword of r3. Flags and r0 hold.
   * ror #8 is fa00 f393. SXTH.W and SXTAB stay closed. */
  static const uint16_t kProg[] = {0xFA00u, 0xF383u, 0x4770u};
  uint8_t mem_buf[32];
  load_halfwords(mem_buf, sizeof(mem_buf), kProg, 3);
  struct {
    uint32_t r0, r3, want;
      ...
      {0x4a0u, 0x59u, 0x4f9u},
      {1u, 0x8000u, 0xffff8001u},
      {5u, 0xffffu, 4u},
      {0u, 0x7fffu, 0x7fffu},
  };
  MangoInsn di;
  if (mango_decode_t32(0xFA00u, 0xF383u, &di) != 0 || di.op != MANGO_OP_XTEND || di.rd != 3 ||
      di.rn != 0 || di.rm != 3 || di.u != 0 || di.b != 1 || di.imm != 0 || di.sets_flags != 0) {
    fprintf(stderr, "FAIL(t32_sxtah): decode op=%d rd=%u rn=%u u=%d b=%d\n", di.op, di.rd, di.rn,
            di.u, di.b);
    return 1;
  }
  if (mango_decode_t32(0xFA00u, 0xFF83u, &di) == 0 || mango_decode_t32(0xFA00u, 0xF38Fu, &di) == 0 ||
      mango_decode_t32(0xFA40u, 0xF383u, &di) == 0) {
    fprintf(stderr, "FAIL(t32_sxtah): PC or SXTAB decoded\n");
    return 1;
  }
  if (mango_decode_t32(0xFA0Fu, 0xF383u, &di) != 0 || di.op != MANGO_OP_XTEND || di.rn != 15 ||
      di.u != 0 || di.b != 1) {
    fprintf(stderr, "FAIL(t32_sxtah): SXTH.W fa0ff383 is Q-OTTD-0cy op=%d\n", di.op);
    return 1;
  }
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    MangoCpu cpu;
    ...
    MangoMemory mem = {mem_buf, sizeof(mem_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    if (rc != 0 || cpu.r[3] != cases[c].want || cpu.r[0] != cases[c].r0 || cpu.cpsr != cpsr_before) {
      fprintf(stderr, "FAIL(t32_sxtah#%u): rc=%d r3=%x want %x cpsr=%x\n", c, rc, cpu.r[3],
              cases[c].want, cpu.cpsr);
      return 1;
    }
  }
  static const uint16_t kRot[] = {0xFA00u, 0xF393u, 0x4770u};
  load_halfwords(mem_buf, sizeof(mem_buf), kRot, 3);
  if (mango_decode_t32(0xFA00u, 0xF393u, &di) != 0 || di.imm != 8 || di.u != 0 || di.b != 1) {
    fprintf(stderr, "FAIL(t32_sxtah ror): imm=%u u=%d b=%d\n", di.imm, di.u, di.b);
    return 1;
  }
  {
    MangoCpu cpu;
    ...
    MangoMemory mem = {mem_buf, sizeof(mem_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    if (rc != 0 || cpu.r[3] != 0xffff8001u || cpu.r[0] != 1u || cpu.cpsr != cpsr_before) {
      fprintf(stderr, "FAIL(t32_sxtah ror): rc=%d r3=%x cpsr=%x\n", rc, cpu.r[3], cpu.cpsr);
      return 1;
    }
  }
  printf("ok: T32 SXTAH r3, r0, r3 (Q-OTTD-0cx)\n");
  return 0;
}

static int test_t32_sxth_w(void) {
  /* Q-OTTD-0cy: sxth.w r8, r8 = fa0f f888 (llvm-mc [0f,fa,88,f8]).
   * r8 = sign-extended low halfword. Flags hold. ror #8 is fa0f f898. */
  static const uint16_t kProg[] = {0xFA0Fu, 0xF888u, 0x4770u};
  uint8_t mem_buf[32];
  load_halfwords(mem_buf, sizeof(mem_buf), kProg, 3);
  struct {
    uint32_t in, want;
  ...
  MangoInsn di;
  if (mango_decode_t32(0xFA0Fu, 0xF888u, &di) != 0 || di.op != MANGO_OP_XTEND || di.rd != 8 ||
      di.rm != 8 || di.rn != 15 || di.u != 0 || di.b != 1 || di.imm != 0) {
    fprintf(stderr, "FAIL(t32_sxth): decode op=%d rd=%u u=%d b=%d\n", di.op, di.rd, di.u, di.b);
    return 1;
  }
  if (mango_decode_t32(0xFA0Fu, 0xFF88u, &di) == 0 || mango_decode_t32(0xFA0Fu, 0xF88Fu, &di) == 0) {
    fprintf(stderr, "FAIL(t32_sxth): PC decoded\n");
    return 1;
  }
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    MangoCpu cpu;
    ...
    MangoMemory mem = {mem_buf, sizeof(mem_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    if (rc != 0 || cpu.r[8] != cases[c].want || cpu.cpsr != cpsr_before) {
      fprintf(stderr, "FAIL(t32_sxth#%u): rc=%d r8=%x want %x\n", c, rc, cpu.r[8], cases[c].want);
      return 1;
    }
  }
  static const uint16_t kRot[] = {0xFA0Fu, 0xF898u, 0x4770u};
  load_halfwords(mem_buf, sizeof(mem_buf), kRot, 3);
  {
    MangoCpu cpu;
    memset(&cpu, 0, sizeof(cpu));
    ...
    MangoMemory mem = {mem_buf, sizeof(mem_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    if (rc != 0 || cpu.r[8] != 0xffff8000u) {
      fprintf(stderr, "FAIL(t32_sxth ror): rc=%d r8=%x\n", rc, cpu.r[8]);
      return 1;
    }
  }
  printf("ok: T32 SXTH.W r8, r8 (Q-OTTD-0cy)\n");
  return 0;
}
...
static int test_t32_ldrd_post_32(void) {
  /* Q-OTTD-0cj: ldrd r2, r3, [r5], #32 = e8f5 2308
   * (llvm-mc [f5,e8,08,23]). Load at the old r5, then r5 += 32. */
  static const uint16_t kProg[] = {0xE8F5u, 0x2308u, 0x4770u};
  uint8_t mem_buf[128];
  memset(mem_buf, 0, sizeof(mem_buf));
  load_halfwords(mem_buf, sizeof(mem_buf), kProg, 3);
  MangoInsn di;
  if (mango_decode_t32(0xE8F5u, 0x2308u, &di) != 0 || di.op != MANGO_OP_LDRD || di.rd != 2 ||
      di.rn != 5 || di.imm != 32u || di.p != 0 || di.u != 1 || di.w != 1) {
    fprintf(stderr, "FAIL(t32_ldrd_post): decode op=%d rd=%u imm=%u p=%d w=%d\n", di.op, di.rd,
            di.imm, di.p, di.w);
    return 1;
  }
  uint32_t base = 64u;
  u32_to_bytes_le(mem_buf + base, 0x33333333u);
  u32_to_bytes_le(mem_buf + base + 4u, 0x44444444u);
  MangoCpu cpu;
  memset(&cpu, 0, sizeof(cpu));
  cpu.cpsr = MANGO_CPSR_T | MANGO_CPSR_C;
  ...
  MangoMemory mem = {mem_buf, sizeof(mem_buf)};
  int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
  if (rc != 0 || cpu.r[2] != 0x33333333u || cpu.r[3] != 0x44444444u || cpu.r[5] != base + 32u ||
      cpu.cpsr != cpsr_before) {
    fprintf(stderr, "FAIL(t32_ldrd_post): rc=%d r2=%x r3=%x r5=%x cpsr=%x\n", rc, cpu.r[2],
            cpu.r[3], cpu.r[5], cpu.cpsr);
    return 1;
  }
  printf("ok: T32 LDRD r2, r3, [r5], #32 (Q-OTTD-0cj)\n");
  return 0;
}
...
static int test_t32_lsr_w_reg(void) {
  /* Q-OTTD-0cn: lsr.w r11, r11, r4 = fa2b fb04 (llvm-mc [2b,fa,04,fb]).
   * Amount is r4[7:0]. S=0, so NZCV hold. Amount 0 leaves r11. */
  static const uint16_t kProg[] = {0xFA2Bu, 0xFB04u, 0x4770u};
  uint8_t mem_buf[32];
  load_halfwords(mem_buf, sizeof(mem_buf), kProg, 3);
  struct {
    uint32_t v, sh, want;
  } cases[] = {
      {0x80000011u, 1u, 0x40000008u},
      {0x80000011u, 0u, 0x80000011u},
      {0xffu, 32u, 0u},
      {0xffu, 40u, 0u},
  };
  MangoInsn di;
  if (mango_decode_t32(0xFA2Bu, 0xFB04u, &di) != 0 || di.op != MANGO_OP_MOV || di.rd != 11 ||
      di.rm != 11 || di.rs != 4 || di.shift_by_reg != 1 || di.shift_type != 1 ||
      di.sets_flags != 0) {
    fprintf(stderr, "FAIL(t32_lsr_w): decode op=%d rd=%u rm=%u rs=%u ty=%u s=%d\n", di.op, di.rd,
            di.rm, di.rs, di.shift_type, di.sets_flags);
    return 1;
  }
  if (mango_decode_t32(0xFA3Bu, 0xFB04u, &di) != 0 || di.sets_flags != 1 || di.shift_type != 1) {
    fprintf(stderr, "FAIL(t32_lsr_w): LSRS fa3bfb04 is Q-OTTD-0ct\n");
    return 1;
  }
  if (mango_decode_t32(0xFA2Bu, 0xFF04u, &di) == 0 || mango_decode_t32(0xFA2Fu, 0xFB04u, &di) == 0 ||
      mango_decode_t32(0xFA2Bu, 0xFB0Fu, &di) == 0) {
    fprintf(stderr, "FAIL(t32_lsr_w): PC decoded\n");
    return 1;
  }
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    MangoCpu cpu;
    ...
    MangoMemory mem = {mem_buf, sizeof(mem_buf)};
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    if (rc != 0 || cpu.r[11] != cases[c].want || cpu.r[4] != cases[c].sh ||
        cpu.cpsr != cpsr_before) {
      fprintf(stderr, "FAIL(t32_lsr_w#%u): rc=%d r11=%x want %x cpsr %x->%x\n", c, rc, cpu.r[11],
              cases[c].want, cpsr_before, cpu.cpsr);
      return 1;
    }
  }
  printf("ok: T32 LSR.W r11, r11, r4 (Q-OTTD-0cn)\n");
  return 0;
}

static int test_t32_lsrs_w_reg(void) {
  /* Q-OTTD-0ct: lsrs.w r0, r4, r1 = fa34 f001 (llvm-mc [34,fa,01,f0]).
   * r0 = r4 >> r1[7:0]. C is the shifter carry. V holds. Amount 0
   * leaves C alone. */
  static const uint16_t kProg[] = {0xFA34u, 0xF001u, 0x4770u};
  uint8_t mem_buf[32];
  load_halfwords(mem_buf, sizeof(mem_buf), kProg, 3);
  struct {
    uint32_t r4, r1, extra, want, nzcv;
  } cases[] = {
      {0x80000011u, 1u, MANGO_CPSR_V, 0x40000008u, MANGO_CPSR_C | MANGO_CPSR_V},
      {0x11u, 0u, MANGO_CPSR_N | MANGO_CPSR_C | MANGO_CPSR_V, 0x11u, MANGO_CPSR_C | MANGO_CPSR_V},
      {0x80000000u, 32u, MANGO_CPSR_V, 0u, MANGO_CPSR_Z | MANGO_CPSR_C | MANGO_CPSR_V},
      {0xffu, 40u, MANGO_CPSR_C | MANGO_CPSR_V, 0u, MANGO_CPSR_Z | MANGO_CPSR_V},
      {0x4480u, 8u, MANGO_CPSR_V, 0x44u, MANGO_CPSR_C | MANGO_CPSR_V},
  };
  MangoInsn di;
  if (mango_decode_t32(0xFA34u, 0xF001u, &di) != 0 || di.op != MANGO_OP_MOV || di.rd != 0 ||
      di.rm != 4 || di.rs != 1 || di.shift_by_reg != 1 || di.shift_type != 1 ||
      di.sets_flags != 1) {
    fprintf(stderr, "FAIL(t32_lsrs_w): decode op=%d rd=%u rm=%u rs=%u ty=%u s=%d\n", di.op, di.rd,
            di.rm, di.rs, di.shift_type, di.sets_flags);
    return 1;
  }
  for (unsigned c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
    ...
    int rc = mango_interp_run(&cpu, &mem, 0xABCDu, 10);
    uint32_t nzcv = cpu.cpsr & (MANGO_CPSR_N | MANGO_CPSR_Z | MANGO_CPSR_C | MANGO_CPSR_V);
    if (rc != 0 || cpu.r[0] != cases[c].want || cpu.r[4] != cases[c].r4 || cpu.r[1] != cases[c].r1 ||
        nzcv != cases[c].nzcv) {
      fprintf(stderr, "FAIL(t32_lsrs_w#%u): rc=%d r0=%x want %x nzcv=%x want %x\n", c, rc, cpu.r[0],
              cases[c].want, nzcv, cases[c].nzcv);
      return 1;
    }
  }
  if (mango_decode_t32(0xFA24u, 0xF001u, &di) != 0 || di.sets_flags != 0 || di.shift_type != 1) {
    fprintf(stderr, "FAIL(t32_lsrs_w): LSR.W S=0 should stay flags-off\n");
    return 1;
  }
  if (mango_decode_t32(0xFA34u, 0xFF01u, &di) == 0 || mango_decode_t32(0xFA3Fu, 0xF001u, &di) == 0 ||
      mango_decode_t32(0xFA34u, 0xF00Fu, &di) == 0) {
    fprintf(stderr, "FAIL(t32_lsrs_w): PC decoded\n");
    return 1;
  }
  printf("ok: T32 LSRS.W r0, r4, r1 (Q-OTTD-0ct)\n");
  return 0;
}
...
static int test_t32_and_w_modimm_reject(void) {
  ...
  /* Q-OTTD-0bk: AND register ea03 0301 = and.w r3,r3,r1. ANDS register is 0cu. */
  ...
  if (mango_decode_t32(0xEA13u, 0x0301u, &di) != 0 || di.op != MANGO_OP_AND || di.sets_flags != 1 ||
      di.rm != 1) {
    fprintf(stderr, "FAIL(t32_and_w_reject): ANDS-reg ea130301 is Q-OTTD-0cu op=%d s=%d\n", di.op,
            di.sets_flags);
    return 1;
  }
  /* Q-OTTD-0af: T32 CLZ fab0 f080 now tip */
...
int main(void) {
  ...
  failures += test_t32_sxtah();
  failures += test_t32_sxth_w();
  ...
  failures += test_t32_lsr_w_reg();
  failures += test_t32_lsrs_w_reg();
